...
class user
{
    ...
    void displayBalance(); // It displays the current balance, and properties in possesion
		...
		const vector<color*>& getColorProperties(string); // Get all properties of an specific color
		...
		const vector<railroad*>& getRailroadProperties(); // It gives access to all railroad properties
		...
		const vector<utility*>& getUtilityProperties();   // It gives access to all utility property
